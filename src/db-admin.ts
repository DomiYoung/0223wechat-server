...
const log = appLogger.child({ module: 'db-admin' });
 ...
 *
 * @param ddlPool - DDL 专用连接池（有 CREATE/ALTER TABLE 权限）
 *                  不传时回退到运行时 pool（向下兼容）
 */
export async function initDB(ddlPool?: mysql.Pool) {
    const db = ddlPool || pool;
    ...
    // 10. app_error_log — 应用错误/告警日志（pino warn/error/fatal 级别自动写入）
    await db.execute(`
    ...
    // 0305 扩展表：消息任务
    // ============================================================
    await ensureMessageTaskTables(db);
    ...
    if (venueRows[0].count === 0) {
        ...
        log.info('default venues seeded');
    }
    ...
    if (caseCatRows[0].count === 0) {
        ...
        log.info('default case categories seeded');
    } else {
    ...
    if (pkgCatRows[0].count === 0) {
        await db.execute(`INSERT INTO package_category (name, slug, sort_order) VALUES
            ...
            ('商务宴会', 'business', 5)
        `);
        log.info('default package categories seeded');
    }

    log.info('database initialization completed');
}

export async function ensureMessageTaskTables(db: mysql.Pool | mysql.PoolConnection = pool) {
    await db.execute(`
        CREATE TABLE IF NOT EXISTS message_task (
            id BIGINT AUTO_INCREMENT PRIMARY KEY,
            channel VARCHAR(50) NOT NULL COMMENT '消息渠道: wechat_subscribe',
            biz_type VARCHAR(50) NOT NULL COMMENT '业务类型: form_submit/new_lead_admin',
            biz_id VARCHAR(64) DEFAULT '' COMMENT '业务ID，如 submitId',
            receiver VARCHAR(100) NOT NULL COMMENT '接收者标识，如 openid',
            template_id VARCHAR(100) NOT NULL COMMENT '模板ID',
            page_path VARCHAR(255) DEFAULT '' COMMENT '跳转页面',
            payload_json JSON NOT NULL COMMENT '消息载荷',
            status ENUM('pending','processing','sent','failed') NOT NULL DEFAULT 'pending' COMMENT '任务状态',
            ...
            created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
            updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
            INDEX idx_channel_status_retry (channel, status, next_retry_at, id),
            INDEX idx_biz_type_id (biz_type, biz_id),
            INDEX idx_receiver_created (receiver, created_at DESC)
        ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='消息任务'
    `);

    await db.execute(`
        CREATE TABLE IF NOT EXISTS message_send_log (
            id BIGINT AUTO_INCREMENT PRIMARY KEY,
            task_id BIGINT NOT NULL COMMENT '消息任务ID',
            channel VARCHAR(50) NOT NULL COMMENT '消息渠道',
        ...
        ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='消息发送流水'
    `);
}
...
async function getCurrentDatabaseName(conn: mysql.PoolConnection): Promise<string | null> {
    const [rows] = await conn.query('SELECT DATABASE() AS db_name') as any;
...
async function dropForeignKeysForColumn(
    ...
    const schemaName = await getCurrentDatabaseName(conn);
    ...
    for (const row of rows || []) {
        try {
            await conn.execute(`ALTER TABLE \`${tableName}\` DROP FOREIGN KEY \`${row.CONSTRAINT_NAME}\``);
            log.info({ tableName, columnName, constraintName: row.CONSTRAINT_NAME }, 'foreign key dropped');
        } catch (err) {
            log.warn({ err, tableName, columnName, constraintName: row.CONSTRAINT_NAME }, 'failed to drop foreign key');
        }
    }
}

export async function cleanupCoreRelationRefs() {
    const conn = await pool.getConnection();
    try {
...
export async function dropLegacyForeignKeys() {
    const conn = await pool.getConnection();
    try {
        const targets = [
            { tableName: 'wedding_case', columnName: 'venue_id' },
            { tableName: 'wedding_case', columnName: 'category_id' },
            { tableName: 'case_image', columnName: 'case_id' },
            { tableName: 'reservation', columnName: 'venue_id' },
            { tableName: 'reservation', columnName: 'case_id' },
            { tableName: 'reservation', columnName: 'city_id' },
            { tableName: 'venue', columnName: 'city_id' },
            { tableName: 'venue', columnName: 'brand_id' },
            { tableName: 'lead_submit_field', columnName: 'submit_id' },
            { tableName: 'package', columnName: 'category_id' },
            { tableName: 'package_image', columnName: 'package_id' },
            { tableName: 'venue_image', columnName: 'venue_id' },
        ];

        for (const target of targets) {
            await dropForeignKeysForColumn(conn, target.tableName, target.columnName);
        }
    } finally {
        conn.release();
    }
...
async function migrateCityData(db: mysql.Pool) {
    ...
    if (migratedCount > 0) {
        log.info({
            cityCount: allCityNames.size,
            venueRows: venueCities.length,
            reservationRows: resCities.length,
        }, 'city migration completed');
    }
}
